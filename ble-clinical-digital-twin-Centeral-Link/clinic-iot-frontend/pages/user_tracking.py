...
_PATIENT_ORBIT = 0.04  # orbit radius around router center


def make_cluster_map(routers_map, devices=None, blueprint_url=None):
    fig = go.Figure()
    floor_plan.draw_clinic(fig)

    # Build router-id → (x, y) lookup
    coord = {
        r["id"]: (r.get("location_x", 0), r.get("location_y", 0))
        for r in (routers_map or [])
        if r.get("id")
    }

    # Faint router reference rings
    if routers_map:
        fig.add_trace(
            go.Scatter(
                x=[r.get("location_x", 0) for r in routers_map],
                y=[r.get("location_y", 0) for r in routers_map],
                mode="markers",
                marker=dict(
                    size=22,
                    color="rgba(88,166,255,0.12)",
                    line=dict(color="rgba(88,166,255,0.35)", width=1.5),
                ),
                text=[r.get("id", "")[:8] for r in routers_map],
                hovertemplate="Router %{text}<extra></extra>",
            )
        )

    # One dot per patient, orbiting their current router
    if devices:
        ...
        for rid, group in groups.items():
            ...
            for i, d in enumerate(group):
                ...
                px.append(rx + _PATIENT_ORBIT * math.cos(angle))
                py.append(ry + _PATIENT_ORBIT * math.sin(angle))
                holder = d.get("holder_name") or "Unassigned"
                labels.append(
                    f"<b>{holder}</b><br>"
                    f"Device: {d.get('name','?')}<br>"
                    f"Room: {d.get('router_name','?')}"
                )
                colors.append("#3fb950" if holder != "Unassigned" else "#e3b341")
        ...
        rows.append(
            html.Tr(
                [
                    ...
                    html.Td(
                        _relative_time(str(d.get("last_record_timestamp") or "")),
                            ...
                            children=[
                                ...
                                html.Button(
                                    ...
                                    style={
                                        ...
                                        "padding": "3px 8px",
                                        "display": (
                                            "" if d.get("holder_name") else "none"
                                        ),
                                    },
                                ),
                            ],
...
def layout():
    ...
    cluster_fig = make_cluster_map(routers_map, devices, blueprint_url)

    free_devices = [d for d in basic_devices if not d.get("patient_id")]
    device_options = [
        {"label": f"{d.get('name','?')} ({d['id'][:8]})", "value": d["id"]}
    ...
    all_patients = api.get_patients()
    patient_options = [{"label": p["name"], "value": p["id"]} for p in all_patients]
    ...
    return html.Div(
        [
            ...
            dcc.Store(id="ut-scanned-mac", data=None),
            dcc.Store(id="ut-modal-device-id", data=None),
            dcc.Store(id="ut-patients-data", data=all_patients),
            html.Div(
                style={
                    "display": "flex",
                    "gap": "12px",
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    html.Div(
                                        "Total Active Devices", className="stat-label"
                                    ),
                            ...
                            html.Div(
                                "from /devices/with-routers-info",
                                style={
                                    "fontSize": "10px",
                                    "color": "var(--text-muted)",
                                },
                    ...
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    html.Div(
                                        "Avg. Session Time", className="stat-label"
                                    ),
                                    html.Span("⏱", style={"fontSize": "18px"}),
                                ],
                            ),
                            ...
                            html.Div(
                                "from /records/hourly-sessions-duration",
                                style={
                                    "fontSize": "10px",
                                    "color": "var(--text-muted)",
                                },
            ...
            html.Div(
                ...
                children=[
                            ...
                            children=[
                                html.Thead(
                                    html.Tr(
                                        [
                                            html.Th("DEVICE"),
                                            html.Th("CONNECTED ROUTER"),
                                            html.Th("PATIENT (HOLDER)"),
                                            html.Th("STATUS"),
                                            html.Th("LAST UPDATE"),
                                            html.Th("ACTIONS"),
                                        ]
                    ...
                    html.Div(
                        f"Showing {min(50,total_users)} of {total_users} — from /devices/with-routers-info",
                        style={
                            "padding": "12px 14px",
                            "fontSize": "11px",
                            "color": "var(--text-muted)",
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            ...
                            # Reassign section — always rendered, shown when modal is open
                            html.Div(
                                id="device-modal-reassign-section",
                                style={"display": "none"},
                                children=[
                                    html.Hr(
                                        style={
                                    ...
                                    html.Div(
                                        "REASSIGN TO PATIENT", className="cl-label"
                                    ),
                                    dcc.Dropdown(
                                        id="device-modal-patient-select",
...
clientside_callback(
    ClientsideFunction(namespace="serial", function_name="scan_patient"),
    Output("ut-scanned-mac", "data"),
    Output("ut-scan-display", "children"),
    Output("ut-scan-status", "children"),
    Input("ut-scan-btn", "n_clicks"),
...
@callback(
    ...
    State("ut-scanned-mac", "data"),
    prevent_initial_call=True,
)
def add_patient(n, name, scanned_mac):
    if not name:
        return html.Div(
    ...
    if not scanned_mac:
        return html.Div(
            "Please scan a BLE device first.",
            className="alert-error",
            style={"marginBottom": "8px"},
        )

    # Re-use existing device if already registered, otherwise auto-create it
    devices = api.get_devices()
    existing = next(
        (d for d in devices if d["id"].upper() == scanned_mac.upper()), None
    )

    if existing:
        if existing.get("patient_id"):
            return html.Div(
                "This device is already assigned to another patient. "
                "Release it first before adding a new patient.",
                className="alert-error",
                style={"marginBottom": "8px"},
            )
        result, err = api.rename_device(existing["id"], name=f"{name}'s device")
        if result is None:
            return html.Div(
                err or "Failed to update existing device.",
                className="alert-error",
                style={"marginBottom": "8px"},
            )
        device_id = existing["id"]
    else:
        result, err = api.create_device(scanned_mac, f"{name}'s device")
        if result is None:
            return html.Div(
                err or "Failed to register device.",
                className="alert-error",
                style={"marginBottom": "8px"},
            )
        device_id = scanned_mac

    result, err = api.create_patient(name, device_id)
    if result is None:
        return html.Div(
            err or "Failed to add patient — check connection.",
            className="alert-error",
            style={"marginBottom": "8px"},
        )
    return html.Div(
...
def toggle_map_mode(rt_n, hist_n, routers_map, devices, blueprint_url):
    is_realtime = ctx.triggered_id == "map-realtime"
    active_devices = devices if is_realtime else []
    active_routers = routers_map if is_realtime else []
    fig = make_cluster_map(active_routers or [], active_devices or [], blueprint_url)
    if not is_realtime:
        # 24h history: show router rings only, no individual patients
        fig.add_trace(
            go.Scatter(
    ...
    return (
        fig,
        "btn-primary" if is_realtime else "btn-outline",
        "btn-outline" if is_realtime else "btn-primary",
    )
...
@callback(
    ...
    Output("ut-modal-device-id", "data"),
    Output("device-modal-reassign-section", "style"),
    Output("device-modal-patient-select", "options"),
    Input({"type": "device-menu-btn", "index": dash.ALL}, "n_clicks"),
    Input("close-device-modal", "n_clicks"),
    State("ut-devices-data", "data"),
    State("ut-patients-data", "data"),
    prevent_initial_call=True,
)
def device_modal(menu_clicks, close_n, devices, patients):
    triggered = ctx.triggered_id
    _no_reassign = {"display": "none"}
    _show_reassign = {"display": "block"}
    patient_options = [{"label": p["name"], "value": p["id"]} for p in (patients or [])]
    if triggered == "close-device-modal":
        return MODAL_HIDDEN, "", [], dash.no_update, None, _no_reassign, []
    ...
    if sessions:
        ...
        sess_fig.update_layout(
            ...
            yaxis=dict(
                gridcolor="#21262d", zeroline=False, tickfont=dict(size=8), title="min"
            ),
        )
        ...
        sessions_block = html.Div(
            (
                ...
                if pid
                else "No patient assigned to this device."
            ),
            style={
                "marginTop": "16px",
                "fontSize": "12px",
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Div(
                                str(device.get("last_record_timestamp") or "—")[:16],
                                style={
                                    "fontWeight": 600,
                                    "marginTop": "6px",
                                    "fontSize": "12px",
...
@callback(
    Output("device-modal-reassign-msg", "children"),
    Input("device-modal-reassign-btn", "n_clicks"),
    State("ut-modal-device-id", "data"),
    State("device-modal-patient-select", "value"),
    prevent_initial_call=True,
)
def reassign_device_cb(n, device_id, patient_id):
    ...
    result, err = api.reassign_device(device_id, patient_id)
    if result is None:
        return html.Div(
            err or "Reassign failed.",
            className="alert-error",
            style={"marginBottom": "4px"},
        )
    return html.Div(
        "Device reassigned successfully.",
        className="alert-success",
        style={"marginBottom": "4px"},
    )
